...
// attributesFromV2 restores Attributes from object.Attribute slice.
//
// All slice elements must not be nil.
func attributesFromV2(as *Attributes, asv2 []*object.Attribute) {
	ln := len(asv2)
	as.SetLen(ln)
...
// attributesWriteToV2 writes Attributes to object.Attribute slice.
//
// Slice length must be at least Len(). Items can be nil.
func attributesToV2(asv2 []*object.Attribute, as Attributes) {
	ind := 0

	as.Iterate(func(a Attribute) bool {
		if asv2[ind] == nil {
			asv2[ind] = new(object.Attribute)
		}
